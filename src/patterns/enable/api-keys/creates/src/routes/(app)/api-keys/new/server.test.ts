...
describe('GET /api-keys/new', () => {
	...
	it('should return a 200 status code', async (context) => {
		const response = await context.agent.get('/api-keys/new' satisfies Match<RouteId>);
...
describe('api keys authenticate as a user', () => {
	it('should create a new api key', async (context) => {
		const apiKey = await createApiKey(context.pb, context.user.id, 'Test API Key');
	...
	it('should give access to the api', async (context) => {
		const response = await context.request.get(`/api/collections/users/records/${context.user.id}`);
