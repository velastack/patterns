...
export const actions = {
	updateTeam: async ({ locals, params, request, cookies }) => {
		...
		try {
			await locals.pb.collection('teams').update(params.id, { name: form.data.name });
		} catch (error) {
			setPocketbaseErrors(form, error);
			return fail(400, { form });
		}

		redirect(303, `/teams/${params.id}`, { type: 'toast', message: 'Team updated' }, cookies);
	},
	removeMember: async ({ locals, params, request, cookies }) => {
		...
		redirect(303, `/teams/${params.id}`, { type: 'toast', message: 'Member removed' }, cookies);
	},
	...
	cancelInvite: async ({ locals, params, request, cookies }) => {
		...
		redirect(303, `/teams/${params.id}`, { type: 'toast', message: 'Invite cancelled' }, cookies);
	},
	deleteTeam: async ({ locals, params, cookies }) => {
		...
		redirect(303, `/teams`, { type: 'toast', message: 'Team deleted successfully' }, cookies);
	}
};
