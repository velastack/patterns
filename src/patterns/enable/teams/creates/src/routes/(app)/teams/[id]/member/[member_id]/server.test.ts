...
describe('POST /teams/[id]/member/[member_id]', () => {
	it('owner should be able to update member role', async (context) => {
		...
		const updatedMember = await context.admin.collection('team_memberships').getOne(member.id);
	...
	it('member should not be able to update member role', async (context) => {
		...
		const unchangedMember = await context.admin.collection('team_memberships').getOne(member.id);
