...
function usage(): never {
  console.error("Usage: npm run demo -- <category> <name> [argv...]");
  console.error("");
  console.error("Examples:");
  console.error("  npm run demo -- enable auth");
  console.error("  npm run demo -- enable api-keys");
  console.error("  npm run demo -- generate form contact name:text email:email");
...
const [category, name, ...patternArgv] = args;
...
// Seed features from pattern.requires so prerequisites are satisfied automatically.
const features: Features = {
  auth: pattern.requires.auth ?? false,
  api: pattern.requires.api ?? false,
  apiKeys: false,
  i18n: false,
  teams: false,
  payments: pattern.requires.payments ?? false,
};

mkdirSync(INTEGRATION_ROOT, { recursive: true });
const tempRoot = mkdtempSync(path.join(INTEGRATION_ROOT, `demo-${slug}-`));
...
console.log(`Applying pattern: ${slug}\n`);
...
const created = result.creates.map((f) => f.path);
const modified = result.modifies.map((f) => f.path);
const deleted = result.deletes.map((f) => f.path);
...
if (components.length) {
  ...
  for (const c of components) console.log(`  * ${c}`);
}
if (packages.length) {
